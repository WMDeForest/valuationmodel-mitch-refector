"""
This module provides functionality for converting ISO 3166-1 alpha-2 country codes to their full country names.

The primary use case is in the Streamlit application where we need to convert country codes from various data sources
(like Spotify audience data) into human-readable country names for display and analysis purposes.

Usage:
    from utils.population_utils.country_code_to_name import country_code_to_name
    
    full_name = country_code_to_name('US')  # Returns 'United States'
    unknown_code = country_code_to_name('XX')  # Returns 'XX' (preserves unknown codes)

Notes:
    - Uses ISO 3166-1 alpha-2 country codes (two-letter codes)
    - Returns the original code if no mapping exists
    - Critical for matching country names in population data and audience analysis
    - Used specifically in fraud detection where we need to match country names exactly
"""
...
def country_code_to_name(code):
    ...
    Args:
        code (str): Two-letter ISO 3166-1 alpha-2 country code (e.g., 'US', 'GB')
    
    Returns:
        str: Full country name if code exists in mapping, otherwise returns the original code
        
    Examples:
        >>> country_code_to_name('US')
        'United States'
        >>> country_code_to_name('XX')  # Unknown code
        'XX'
    """
    country_map = {
        ...
        'PL': 'Poland', 'PT': 'Portugal', 'PR': 'Puerto Rico', 'QA': 'Qatar', 'MK': 'Republic of North Macedonia',
        ...
        'SY': 'Syrian Arab Republic', 'TW': 'Taiwan', 'TJ': 'Tajikistan', 'TZ': 'Tanzania, United Republic of', 'TH': 'Thailand',
